...
function init() {
  ...
  if (!window.CS || window.CS.staff_role !== 'admin') { location.href = 'index.html'; return; }
...
function ldClientes(){
  ...
  el.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--muted)"><span class="ld"></span> Carregando clientes...</td></tr>';
  
  // Query profiles directly (works with anon key + allow_all policy)
  q(function(s){
    return s.from('profiles').select('*').order('created_at', {ascending:false});
  }).then(function(res){
    var profiles = res.data || [];
    allClients = profiles;
    rClientes(profiles);
    // Update stat card
    var st4 = g('st4'); if(st4) st4.textContent = profiles.length;
  }).catch(function(e){
    // Fallback: try to get clients from cases table
    q(function(s){
      return s.from('cases').select('id, client_id, profiles(id,full_name,email,phone,cpf,city,state,created_at)').order('created_at',{ascending:false});
    }).then(function(res2){
      var seen = {};
      var merged = [];
      (res2.data||[]).forEach(function(c){
        if(c.profiles && !seen[c.profiles.id]){
          seen[c.profiles.id] = true;
          merged.push(c.profiles);
        }
      });
      allClients = merged;
      rClientes(merged);
    }).catch(function(e2){
      el.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem">' +
        '<div style="color:#f08080;margin-bottom:.5rem">Erro ao carregar: ' + esc(e.message) + '</div>' +
        '<div style="font-size:.75rem;color:var(--muted)">Verifique as políticas RLS no Supabase (allow_all para anon)</div>' +
        '</td></tr>';
    });
  });
}
...
function salvarNews(){
  ...
  if(!ti){ shAl('al-nws','Preencha o título da notícia.','e'); return; }
  if(!co && !admNewsFile){ shAl('al-nws','Escreva o conteúdo ou envie um arquivo.','e'); return; }
  var slug = ti.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'') + '-' + Date.now();
  var nm = window.CS ? window.CS.staff_name : 'Admin';
...
[Arquivo: ' + (admNewsFile ? admNewsFile.name : '') + '](' + fileUrl + ')';
    ...
    if(res && res.error){ shAl('al-nws',res.error.message,'e'); return; }
    cM('m-news');
    admNewsFile = null;
    var fn = g('adm-news-file-name'); if(fn) fn.textContent='';
    var inp = g('finp-adm-news'); if(inp) inp.value='';
    ['nw-ti','nw-ex','nw-co'].forEach(function(id){var e=g(id);if(e)e.value='';});
    lgAct('create_news','Notícia publicada: '+ti,'admin');
    ldNoticias();
  }).catch(function(e){ shAl('al-nws',e.message||'Erro ao publicar.','e'); });
}

var admNewsFile = null;
function hAdmNewsFile(files){
  if(!files||!files.length) return;
  admNewsFile = files[0];
  var el = g('adm-news-file-name');
  if(el) el.textContent = '📄 ' + admNewsFile.name + ' (' + (admNewsFile.size/1024).toFixed(0) + 'KB)';
}
function uploadAdmNewsFile(file){
