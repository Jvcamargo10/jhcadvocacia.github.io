...
function ldClientes(){
  ...
  // Load ALL profiles (clients registered via portal AND cases with client data)
  Promise.all([
    q(function(s){return s.from('profiles').select('*').order('created_at',{ascending:false});}).catch(function(){return {data:[]};}),
    q(function(s){return s.from('cases').select('client_id, profiles(id,full_name,email,phone,cpf,city,created_at)').order('created_at',{ascending:false});}).catch(function(){return {data:[]};})
  ]).then(function(results){
    var profiles = results[0].data || [];
    var cases = results[1].data || [];
    // Merge: clients from profiles + any from cases not already in profiles
    var seen = {};
    var merged = [];
    ...
    cases.forEach(function(c){
      if(c.profiles && !seen[c.profiles.id]){
        seen[c.profiles.id]=true; merged.push(c.profiles);
      }
    });
    allClients = merged;
    rClientes(merged);
    // Update stat card
    var st4 = g('st4'); if(st4) st4.textContent = merged.length;
  }).catch(function(e){
    el.innerHTML='<tr><td colspan="6" style="text-align:center;padding:2rem;color:#f08080">Erro ao carregar clientes: '+esc(e.message)+'</td></tr>';
  });
}
