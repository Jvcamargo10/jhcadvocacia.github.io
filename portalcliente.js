...
function init() {
  initSB();
  // Load news for everyone (even without session)
  showS('noticias');
  ldNews();

  if (!SB_OK) {
    // SB not available - show portal in read-only mode
    var nm = document.getElementById('sb-nm'); if(nm) nm.textContent = 'Visitante';
    return;
  }

  sb.auth.getSession().then(function(res) {
    if (!res.data || !res.data.session || !res.data.session.user) {
      // No session - show portal partially (news visible, rest needs login)
      var nm = document.getElementById('sb-nm'); if(nm) nm.textContent = 'Faça Login';
      return;
    }
    ...
    var nm = document.getElementById('sb-nm'); if(nm) nm.textContent = 'Erro de conexão';
  });
}
...
function ldNews() {
  ...
  // Show loading inside a proper block container
  el.style.display = 'block';
  el.innerHTML = '<div class="empty"><div class="empty-i" style="font-size:2rem;opacity:.5">⏳</div><div class="empty-t">Carregando notícias...</div></div>';

  if (!SB_OK && !initSB()) {
    el.innerHTML = '<div class="empty"><div class="empty-i">📡</div><div class="empty-t">Sem conexão com o servidor.<br>Verifique sua internet e recarregue a página.</div></div>';
    return;
  }
  ...
  q(function(s) {
    ...
    if (!ns.length) {
      ...
      el.innerHTML =
        '<div class="empty">' +
        '<div class="empty-i" style="font-size:3rem;opacity:.35">📰</div>' +
        '<div class="empty-t" style="font-size:.92rem">' +
        '<strong style="color:var(--white);display:block;margin-bottom:.5rem">Nenhuma notícia publicada ainda</strong>' +
        'Em breve a equipe do escritório trará novidades jurídicas e informações relevantes para você!' +
        '</div></div>';
      return;
    }
    // Has news - switch back to grid
    ...
    el.innerHTML = ns.map(function(n, i) {
      ...
      var cat = n.category || '';
      var hasFile = !!n.file_url;
      var fileBtn = hasFile
        ? '<a href="' + n.file_url + '" target="_blank" class="btn btn-o btn-sm" style="font-size:.69rem;margin-top:.5rem">📎 Ver Arquivo</a>'
        : '';
      var authorTxt = n.author_name
      ...
      return '<div class="news-card">' +
        ...
        '<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.35rem;margin-top:.5rem">' +
    ...
    el.innerHTML =
      ...
      '<div class="empty-t">Erro ao carregar notícias: ' + esc(e.message) + '<br>' +
      '<span style="font-size:.75rem;color:var(--muted)">Verifique as permissões RLS no Supabase ou tente recarregar a página.</span>' +
      '</div></div>';
  });
}
...
// ── PROCESS LIST ─────────────────────────────────────────
function ldProcs() {
  var el = document.getElementById('proc-list'); if (!el || !window.CU) return;
  ...
  q(function(s) {
    ...
    _allProcsCache = res.data || [];
    _renderProcList(_allProcsCache);
  }).catch(function(e) {
    el.innerHTML = '<div class="empty"><div class="empty-i">⚠️</div><div class="empty-t">Erro: ' + esc(e.message) + '</div></div>';
  });
}
...
function filterProcs(status) {
  ...
  if (!status) { _renderProcList(_allProcsCache); return; }
  _renderProcList(_allProcsCache.filter(function(c){ return c.status === status; }));
}

function _renderProcList(list) {
  var el = document.getElementById('proc-list'); if (!el) return;
  if (!list.length) {
    el.innerHTML = '<div class="empty"><div class="empty-i">📁</div><div class="empty-t">Nenhum processo nesta categoria.<br><span class="flk" onclick="navTo('novo')">Solicitar novo serviço →</span></div></div>';
    return;
  }
  el.innerHTML = list.map(function(c) {
    return '<div class="pcard" onclick="viewProc('' + c.id + '')">' +
      ...
      (c.lawyer_name ? '<span style="color:var(--gold)">⚖ ' + esc(c.lawyer_name) + '</span>' : '<span style="color:var(--muted)">Aguardando advogado</span>') +
      '<span class="pc-link">Ver detalhes →</span></div></div>';
  }).join('');
}

document.addEventListener('DOMContentLoaded', init);
// hFiles is in shared.js; this override adds AI analysis
// Called via: onchange="hFiles(this.files);analisarDocumento(this.files[0])"
// NOT redefined here to avoid conflict
