...
function ldNews() {
  ...
  q(function(s) {
    ...
    var ns = res.data || [];
    if (!ns.length) {
      el.innerHTML = '<div class="empty"><div class="empty-i">📰</div><div class="empty-t">Nenhuma notícia publicada ainda.</div></div>';
      return;
    }
    el.innerHTML = ns.map(function(n) {
      return '<div style="background:var(--navy2);border:1px solid var(--border);border-radius:var(--r2);overflow:hidden;transition:all .2s">' +
        '<div style="background:linear-gradient(135deg,var(--navy3),var(--navy4));height:90px;display:flex;align-items:center;justify-content:center;font-size:2rem">📄</div>' +
        '<div style="padding:1.1rem">' +
        (n.category ? '<div style="font-size:.63rem;color:var(--gold);text-transform:uppercase;letter-spacing:.12em;margin-bottom:.4rem">' + esc(n.category) + '</div>' : '') +
        '<div style="font-size:.93rem;font-weight:700;color:var(--white);margin-bottom:.4rem;line-height:1.3">' + esc(n.title) + '</div>' +
        (n.excerpt ? '<div style="font-size:.79rem;color:var(--muted);line-height:1.62;margin-bottom:.65rem">' + esc(n.excerpt) + '</div>' : '') +
        '<div style="font-size:.67rem;color:var(--muted)">' + fdatefull(n.published_at || n.created_at) + '</div>' +
        '</div></div>';
    }).join('');
  }).catch(function(e) {
